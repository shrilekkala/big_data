...
class CFG:
    ...
    token_name = "microsoft/deberta-large"
...
class FeedbackModel(nn.Module):
    def __init__(self, model_name, pretask_model_path):
        ...
        self.deberta.load_state_dict(state, strict=False)
        # gradient checkpointing
        if CFG.gradient_checkpoint:
            self.deberta.gradient_checkpointing_enable()
            print(f"Gradient Checkpointing: {self.model.is_gradient_checkpointing}")
        if  CFG.freezing:
            freeze(self.deberta.embeddings)
            freeze(self.deberta.encoder.layer[:6])
...
for fold in CFG.train_folding:
    ...
    pretask_model_path = f'/root/feedback/oldtrain/fold{fold}/pytorch_model.bin'
    model = FeedbackModel(CFG.model_name, pretask_model_path)
    #model = apply_mixout(model, p=0.12)
    model.to(CFG.device)
