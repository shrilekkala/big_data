...
### d) Relationship between categorical features and y
```{r}
# Create the box plot
b1<-ggplot(googleplaystore, aes(x = Type, y = Rating)) +
  geom_boxplot(outlier.color = "darkred", outlier.shape = 1,fill = "darkred") +  # Red for outliers
...
ggplot(googleplaystore, aes(x = Category, y = Rating)) +
  ...
  labs(title = "Distribution of Ratings by Category in Google Play Store",
       ...
       y = "Rating") +
  theme_minimal() +
  theme(axis.text.x = element_text(size = 6,angle = 45, hjust = 1, vjust = 1))  # Rotate x-axis labels for better visibility
```
