...
x <- model.matrix(~ Reviews + Size + Installs + Price + Days_Since_Update + Category + Type + Content.Rating + Genres - 1, data = ratings_data)
# Response variable
...
kable(coefficients_df, caption = "Highest and Lowest Coefficients from LASSO Model", row.names = FALSE) |>
  kable_styling(bootstrap_options = c("striped", "hover"))
```

From looking at the coefficients with the largest positive and negative values, we observe that apps in the categories: Events, Personalization, and Books and Reference have a positive association with higher ratings. This might mean that such categories have higher user satisfaction, or there might be other factors such as having less competition compared to other categories.
...
Finally, another interesting category of note is "Dating" which also has a large negative impact, and again this might be due to user dissatisfaction with the service, and makes sense considering the complicated and competitive nature of such apps.
